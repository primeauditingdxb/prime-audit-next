    ...
    <div>
      <form className="space-y-4" onSubmit={onSubmit} noValidate>
        ...
        <div>
          ...
          <input id="name" name="name" value={name} onChange={(e) => { setName(e.target.value); setTouched(t => ({ ...t, name: true })); validate() }} onBlur={() => { setTouched(t => ({ ...t, name: true })); validate() }} className={`mt-1 block w-full border rounded px-3 py-2 ${errors.name ? 'border-red-300' : ''} text-muted`} placeholder="Your name" />
          {errors.name && <p className="text-xs text-red-600 mt-1">{errors.name}</p>}
        </div>

        <div>
          <label className="block text-sm font-medium text-primary-dark" htmlFor="email">Email</label>
          <input id="email" name="email" type="email" value={mail} onChange={(e) => { setMail(e.target.value); setTouched(t => ({ ...t, email: true })); validate() }} onBlur={() => { setTouched(t => ({ ...t, email: true })); validate() }} className={`mt-1 block w-full border rounded px-3 py-2 ${errors.email ? 'border-red-300' : ''} text-muted`} placeholder="you@example.com" />
          {errors.email && <p className="text-xs text-red-600 mt-1">{errors.email}</p>}
        </div>

        <div>
          <label className="block text-sm font-medium text-primary-dark" htmlFor="message">Message</label>
          <textarea id="message" name="message" value={message} onChange={(e) => { setMessage(e.target.value); setTouched(t => ({ ...t, message: true })); validate() }} onBlur={() => { setTouched(t => ({ ...t, message: true })); validate() }} className={`mt-1 block w-full border rounded px-3 py-2 ${errors.message ? 'border-red-300' : ''} text-muted`} rows={5} placeholder="How can we help?"></textarea>
